...
GMM::GMM(unsigned int K) : mK(K) { mGaussians = new GaussianPDF[mK]; }
GMM::~GMM() {
  if (mGaussians) delete[] mGaussians;
}
Real GMM::p(Color c) {
  Real result = 0;
  if (mGaussians) {
...
Real GMM::p(unsigned int i, Color c) {
  ...
  if (mGaussians[i].pi > 0) {
    if (mGaussians[i].determinant > 0) {
      ...
      Real d = r * (r * mGaussians[i].inverse[0][0] +
                    g * mGaussians[i].inverse[1][0] +
                    b * mGaussians[i].inverse[2][0]) +
               g * (r * mGaussians[i].inverse[0][1] +
                    g * mGaussians[i].inverse[1][1] +
                    b * mGaussians[i].inverse[2][1]) +
               b * (r * mGaussians[i].inverse[0][2] +
                    g * mGaussians[i].inverse[1][2] +
                    b * mGaussians[i].inverse[2][2]);
...
int GMM::Build(double** data, uint nrows) {
  ...
  kcluster(nclusters, nrows, ncols, data, mask, weight, transpose, npass,
           method, dist, clusterid, &error, &ifound);
  GaussianFitter* gaussianFitter = new GaussianFitter[mK];
  for (i = 0; i < nrows; i++)
    gaussianFitter[clusterid[j]].add(
        Color((float)data[i][0], (float)data[i][1], (float)data[i][2]));
